...
def test_sql_planning_prompt_uses_selected_candidate_and_requires_traceability():
    scenario = load_scenario(SCENARIO)
    prompt = build_planning_prompt(scenario=scenario, relative_scenario=SCENARIO, candidate_query_name="regression")
    ...
    assert "execution_capabilities" in prompt
    assert "deferred/future research" in prompt
...
def test_sql_planning_prompt_rejects_unknown_candidate_query():
    ...
    with pytest.raises(ValueError, match="Unknown SQL candidate query"):
        build_planning_prompt(scenario=scenario, relative_scenario=SCENARIO, candidate_query_name="does-not-exist")
